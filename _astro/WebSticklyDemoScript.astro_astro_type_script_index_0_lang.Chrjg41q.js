...
Press Alt + N to try it out!`,x:e>800?e-320:20,y:window.innerHeight+240}];const o=t.map(s=>{const i=localStorage.getItem(`ws_demo_note_${s.id}`);if(i){const r=JSON.parse(i);return{...s,...r,metadata:{...s.metadata,...r.metadata}}}return s}),a=`ws_demo_custom_ids_${window.location.pathname}`,n=JSON.parse(localStorage.getItem(a)||"[]").map(s=>{const i=localStorage.getItem(`ws_demo_note_${s}`);return i?JSON.parse(i):null}).filter(Boolean);window.WS.NoteRenderer.renderAll([...o,...n])}function u(){window.addEventListener("keydown",e=>{e.altKey&&(e.key==="n"||e.key==="N")&&(e.preventDefault(),window.WS.NoteManager.create())})}document.addEventListener("DOMContentLoaded",()=>{g(),u()});
